...
class MMFTorchRamanAmplifierWithASE(torch.nn.Module):
    def __init__(
        self,
        length,
        steps,
        ...
        self.reference_bandwidth_nm = reference_bandwidth

        if isinstance(signal_wavelength, np.ndarray):
            signal_wavelength = torch.from_numpy(signal_wavelength).float()

        central_wavelength = float(
            (signal_wavelength.max() + signal_wavelength.min()) / 2
        )

        reference_bandwidth_m = self.reference_bandwidth_nm * 1e-9  # Convert in meters
        w_a = central_wavelength - reference_bandwidth_m / 2
        w_b = central_wavelength + reference_bandwidth_m / 2
        f_a = lambda2nu(w_a)
        f_b = lambda2nu(w_b)

        self.reference_bandwidth_hz = np.abs(f_a - f_b)
    ...
    def ode(
        ...
        gain_factor = torch.matmul(gain_matrix, P_.view(batch_size, -1, 1))
        dPowerdz = (
            (-losses.view(batch_size, -1, 1) + gain_factor) * P_.view(batch_size, -1, 1)
        ).view(batch_size, -1)
        ...
        dASEdz = (
            (-losses_ase.view(batch_size, -1, 1) + gain_factor[:, -num_ase:])
            * P_ase.view(batch_size, -1, 1)
            + gain_factor_ase_
            * 2
            * h_planck
            * frequencies.view(batch_size, -1, 1)
            * reference_bandwidth
        ).view(batch_size, -1)

        return torch.cat((dPowerdz, dASEdz), dim=1) * direction
    ...
    def forward(self, x):
        ...
        solution = torch_rk4(
            ...
            G_ase,
            ase_frequencies,
            self.reference_bandwidth_hz,
            self.num_channels,
            self.num_pumps,
            self.modes,
        ).view(-1, num_freqs + self.num_channels, self.modes)

        power_spectrum = solution[:, :num_freqs, :]
        pump_spectrum = power_spectrum[:, : self.num_pumps, :].clone()
        signal_spectrum = power_spectrum[:, self.num_pumps :, :]
        ase_spectrum = solution[:, -self.num_channels :, :].clone()

        if self.counterpumping:
            pump_initial_power = pump_spectrum
            return signal_spectrum, ase_spectrum, pump_initial_power
        else:
            return signal_spectrum, ase_spectrum
